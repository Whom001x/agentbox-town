...
Long-term memory now defaults to the resident's first-person self experience instead of system templates such as `This person`, `Agent tends`, or `Stable habit`. `beliefMemory`, `habitMemory`, `preferenceMemory`, `episodicMemory`, and `relationshipMemory` are expressed as what the resident believes, remembers, prefers, or thinks about another person. Existing saves run a memory-perspective migration on load to clean English template residue and add conservative first-person episodic anchors from existing identity and goal sources when older residents have no episodic memory yet.

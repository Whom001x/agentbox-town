...
function publicConfig() {
  ensureKeyHealth();
  const effectiveMaxActionsPerCycle = Math.min(aiConfig.maxActionsPerCycle, Math.max(1, aiConfig.apiKeys.length) * aiConfig.maxConcurrentPerKey);
  return {
    aiEnabled: aiConfig.apiKeys.length > 0,
...
function nextApiKey() {
  ensureKeyHealth();
  if (!aiConfig.apiKeys.length) return "";
  const now = Date.now();
  let selected = -1;
...
async function callAi(task, payload) {
  ...
  metrics.total += 1;
  metrics.inFlight += 1;
  keyHealth[selectedKey.index].inFlight += 1;
  const callLog = pushCallLog({
    task,
    model: selectedModel,
    keyIndex: selectedKey.index + 1,
  ...
  metrics.lastStatus = `running:k${selectedKey.index + 1}:${selectedModel}`;
  metrics.lastError = "";
  ...
  try {
    ...
    if (epoch === metricsEpoch) {
      metrics.success += 1;
      metrics.lastStatus = `success:k${selectedKey.index + 1}`;
      markKeySuccess(selectedKey.index, Date.now() - started);
    }
    ...
    if (epoch === metricsEpoch) {
      metrics.failure += 1;
      metrics.lastStatus = `failed:k${selectedKey.index + 1}`;
      metrics.lastError = error.message.slice(0, 240);
      markKeyFailure(selectedKey.index, error, Date.now() - started);
    }
    ...
    if (epoch === metricsEpoch) {
      ...
      keyHealth[selectedKey.index].inFlight = Math.max(0, keyHealth[selectedKey.index].inFlight - 1);
...
async function handleApi(req, res) {
  ...
  if (apiPath === "/api/metrics" && req.method === "GET") {
    send(res, 200, { ...metrics, model: aiConfig.model, keyCount: aiConfig.apiKeys.length, keyHealth: publicKeyHealth() });
    return;
  }
...
http.createServer((req, res) => {
  ...
  console.log(aiConfig.apiKeys.length ? `AI enabled: ${aiConfig.model}, ${aiConfig.apiKeys.length} key(s)` : "AI disabled: configure API key in UI.");
});

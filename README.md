...
长期记忆现在默认以角色第一人称保存，不再使用 `This person`、`Agent tends`、`Stable habit` 等系统模板。`beliefMemory`、`habitMemory`、`preferenceMemory`、`episodicMemory` 和 `relationshipMemory` 会表达为“我相信……”、“我习惯……”、“我记得……”、“我觉得和某人……”。旧存档加载时会运行记忆视角迁移，清理英文模板残留，并在没有经历记忆的旧角色上用已有身份和目标来源生成保守的第一人称经历锚点。
